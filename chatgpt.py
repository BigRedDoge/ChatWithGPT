...
class ChatGPT:
    ...
    def get_response(self):
        ...
        while response is None:
            ...
            """ 
            first_check = self.driver.find_element(By.XPATH, xpath).text
            
            if len(first_check) > 0:
                time.sleep(1)
                second_check = self.driver.find_element(By.XPATH, xpath).text
    ...
    def start_session(self):
        ...
        time.sleep(0.5)
        email.send_keys(self.settings['credentials']['email'])

        password = wait.until(EC.element_to_be_clickable((By.XPATH, '//*[@id="password"]')))
        time.sleep(0.5)
        password.send_keys(self.settings['credentials']['password'])
...
if __name__ == '__main__':
    ...
    chat.start_session()
    time.sleep(5)
    chat.send_chat('Hello, your name is Jarvis. You are my personal assistant.  You will respond to my questions and commands.')
    time.sleep(5)
    from text2speech import TTS
    TTS.say(chat.get_response())
    time.sleep(5)
    chat.new_session()
    time.sleep(5)
